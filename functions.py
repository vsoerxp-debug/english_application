...
def create_chain(system_template, use_separate_memory=False):
    """
    LLMによる回答生成用のChain作成
    Args:
        system_template: システムプロンプト
        use_separate_memory: Trueの場合、独立したMemoryを使用（評価用）
    """

    # 評価用の場合は独立したMemoryを使用
    if use_separate_memory:
        evaluation_memory = ConversationSummaryBufferMemory(
            llm=st.session_state.llm,
            max_token_limit=500,  # 評価用は短めに
            return_messages=True
        )
        chain_memory = evaluation_memory
    else:
        chain_memory = st.session_state.memory
