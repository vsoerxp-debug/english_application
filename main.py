    ...
    if st.session_state.mode == ct.MODE_3 and (st.session_state.dictation_button_flg or st.session_state.dictation_count == 0 or st.session_state.dictation_chat_message):
        ...
        # チャット入力時の処理
            ...
            with st.spinner('評価結果の生成中...'):
                # format the base evaluation template with inputs
                eval_template = ct.SYSTEM_TEMPLATE_EVALUATION.format(
                    llm_text=st.session_state.problem,
                    user_text=st.session_state.dictation_chat_message,
                    user_level=st.session_state.englv
                )
                # combine few-shot examples and CoT helper to improve evaluation quality
                combined_template = ct.SYSTEM_TEMPLATE_FEW_SHOT + "\n" + ct.SYSTEM_TEMPLATE_COT + "\n" + eval_template
                st.session_state.chain_evaluation = ft.create_chain(combined_template, use_separate_memory=True)
